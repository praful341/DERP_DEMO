...
{
    public partial class FrmMFGMixSplitLotting : DevExpress.XtraEditors.XtraForm
    {
        #region Data Member
                ...
                try
                {
                    ...
                    if (IntMixEntry == 0 && lblMode.Text == "NEW")
                    {
                        ...
                        foreach (DataRow drw in m_dtbLotMixSplit.Rows)
                        {
                            ...
                            if (Val.ToString(drw["lotting_department"]) == "RUSSIAN" || Val.ToString(drw["lotting_department"]) == "4P")
                            {
                    ...
                    if ((IntMixEntry == 0 && lblMode.Text == "EDIT") || (IntMixEntry == 1 && lblMode.Text == "EDIT"))
                    {
                        if (chkIsLotingDepartment.Checked == true)
                        {
                            ...
                            foreach (DataRow drw in m_dtbLotMixSplit.Rows)
                            {
                                if (Val.ToInt(drw["final_lot_id"]) != 0)
                                {
                                    ...
                                    objMFGMixSplitProperty.lot_srno = Val.ToInt(drw["lot_srno"]);
                                    objMFGMixSplitProperty.is_repeat = chkIsRepeat.Checked == true ? 1 : 0;
                                    objMFGMixSplitProperty.process_id = Val.ToInt(lueProcess.EditValue);
                                    objMFGMixSplitProperty.sub_process_id = Val.ToInt(lueSubProcess.EditValue);
                        ...
                        //if (ChkIsProcessUpdate.Checked == true)
                        //{
                        //    foreach (DataRow drw in m_dtbLotMixSplit.Rows)
                        //    {
                        //        if (Val.ToInt(drw["final_lot_id"]) != 0)
                        //        {
                        //            objMFGMixSplitProperty.lot_srno = Val.ToInt64(drw["lot_srno"]);
                        //            objMFGMixSplitProperty.process_id = Val.ToInt64(lueProcess.EditValue);
                        //            objMFGMixSplitProperty.sub_process_id = Val.ToInt64(lueSubProcess.EditValue);
                        //            objMFGMixSplitProperty.old_process_id = Val.ToInt64(drw["process_id"]);
                        //            objMFGMixSplitProperty.old_sub_process_id = Val.ToInt64(drw["sub_process_id"]);
                        //            IntRes = objMFGMixSplit.Update_LottingProcess(objMFGMixSplitProperty, DLL.GlobalDec.EnumTran.Continue, Conn);
                        //            break;
                        //        }
                        //    }
                        //}
                        ...
                        if ((Mix_Data.Rows.Count == 1 && m_dtbLotMixSplit.Rows.Count >= 1) || (Mix_Data.Rows.Count == 1 && m_dtbLotMixSplit.Rows.Count == 1))
                        {
                            ...
                            foreach (DataRow drw in Mix_Data.Rows)
                            {
                                ...
                                foreach (DataRow DRow in m_dtbLotMixSplit.Rows)
                                {
                                    ...
                                    if (Val.ToString(DRow["lotting_department"]) == "RUSSIAN" || Val.ToString(DRow["lotting_department"]) == "4P")
                                    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            FrmSearchLotting.FrmMFGMixSplitLotting = this;
        ...
        public void FillGrid(int UnionId, string TransType)
        {
            ...
            if (DTab_StockData.Rows.Count > 0)
            {
                ...
                if (Val.ToInt(DTab_StockData.Rows[0]["is_repeat"]) == 0)
                {
                    chkIsRepeat.Checked = false;
                }
                else
        ...
        public void GetPendingStock()
        {
            try
            {
                if (Save_Validate())
                {
                    ...
                    FrmStockConfirm.FrmMFGMixSplitLotting = this;
