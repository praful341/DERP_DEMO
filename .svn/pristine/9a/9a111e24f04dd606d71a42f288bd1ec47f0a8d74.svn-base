    ...
    public partial class FrmUserMaster : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void FrmUserMaster_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Global.LOOKUPParty(luePartyId);

                DataTable DTabEmployee = ObjFillCombo.FillCmb(FillCombo.TABLE.Employee_Master);
                DTabEmployee.DefaultView.Sort = "employee_name";
                DTabEmployee = DTabEmployee.DefaultView.ToTable();

                lueEmpId.Properties.DataSource = DTabEmployee;
                lueEmpId.Properties.ValueMember = "employee_id";
                lueEmpId.Properties.DisplayMember = "employee_name";
                lueEmpId.ClosePopup();

                // Global.LOOKUPEmployee(lueEmpId);
                Global.LOOKUPUserType(lueUserType);
            ...
            try
            {
                if (e.RowHandle >= 0)
                {
                    if (e.Clicks == 2)
                    {
                        ...
                        lueEmpId.EditValue = Val.ToInt64(Drow["employee_id"]);
        ...
        private bool ValidateDetails()
        {
            ...
            try
            {
                ...
                if (!objUser.EmpIdExists(Val.ToInt(lueEmpId.EditValue), Val.ToInt64(lblMode.Tag)).ToString().Trim().Equals(string.Empty))
                {
                    lstError.Add(new ListError(5, "Already Create User Name of this Employee."));
                    if (!blnFocus)
                    {
                        blnFocus = true;
                        lueEmpId.Focus();
