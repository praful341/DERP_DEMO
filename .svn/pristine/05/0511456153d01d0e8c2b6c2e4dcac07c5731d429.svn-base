...
{
    public partial class FrmMFGLotSplit : DevExpress.XtraEditors.XtraForm
    {
        #region Data Member
                ...
                if (m_blnsave)
                {
                    ...
                    if (chkIsSplit.Checked == false)
                    {
                        ...
                        foreach (DataRow Dr in distinctValues.Rows)
                        {
                            ...
                            if ((Val.ToDecimal(os_carat) + Val.ToDecimal(plus_carat)) != (Val.ToDecimal(loss_carat) + Val.ToDecimal(carat)))
                            {
                    ...
                    //if ((m_balcarat + Val.ToDecimal(dgvLotSplit.Columns["plus_carat"].SummaryText)) < (Val.ToDecimal(dgvLotSplit.Columns["plus_carat"].SummaryText) + Val.ToDecimal(dgvLotSplit.Columns["loss_carat"].SummaryText) + Val.ToDecimal(dgvLotSplit.Columns["carat"].SummaryText) + Val.ToDecimal(dgvLotSplit.Columns["rr_carat"].SummaryText)))
                    //{
        ...
        private void dgvLotSplit_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            try
            {
                if (e.RowHandle >= 0)
                {
                    if (e.Clicks == 2)
                    {
                        DataRow Drow = dgvLotSplit.GetDataRow(e.RowHandle);
                        btnAdd.Text = "&Update";
                ...
                try
                {
                    ...
                    foreach (DataRow drw in m_dtbLotSplitProcess.Rows)
                    {
                        ...
                        if (count == New_Count)
                        {
                            ...
                            objMFGLotSplitProperty.recieve_carat = Val.ToDecimal(drw["os_carat"]) - (TotCarat + Val.ToDecimal(drw["loss_carat"]));
                            objMFGLotSplitProperty.total_loss_carat = Val.ToDecimal(dgvLotSplit.Columns["loss_carat"].SummaryText);
                            objMFGLotSplitProperty.total_plus_carat = Val.ToDecimal(dgvLotSplit.Columns["plus_carat"].SummaryText);
                            New_Count = 0;
                            TotCarat = 0;
                        }
                        else
        ...
        private bool GenerateLotSplitDetails()
        {
            ...
            try
            {
                ...
                grdLotSplit.DataSource = m_dtbLotSplitProcess;
                grdLotSplit.Refresh();
            }
            catch (Exception ex)
            {
                ...
                else if (btnAdd.Text == "&Update")
                {
                    if (m_dtbLotSplitProcess.Select("sr_no ='" + Val.ToInt(m_update_srno) + "'").Length > 0)
                    {
                        for (int i = 0; i < m_dtbLotSplitProcess.Rows.Count; i++)
                        {
                            ...
                            {
                                if (m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["cut_no"].ToString() == m_cut_no.ToString())
                                {
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["lot_id"] = Val.ToString(txtLotID.Text);
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["sieve_name"] = Val.ToString(lueRoughSieve.Text);
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["rough_sieve_id"] = Val.ToInt(lueRoughSieve.EditValue);
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["pcs"] = Val.ToInt(txtPcs.Text).ToString();
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["carat"] = Val.ToDecimal(txtCarat.Text).ToString();
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["loss_carat"] = Val.ToDecimal(txtWeightLoss.Text).ToString();
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["plus_carat"] = Val.ToDecimal(txtWeightPlus.Text).ToString();
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["rate"] = Val.ToDecimal(txtRate.Text);
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["amount"] = Val.ToDecimal(txtAmount.Text);
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["rough_cut_id"] = Val.ToInt32(lueCutNo.EditValue);
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["kapan_id"] = Val.ToInt32(lueKapan.EditValue);
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["os_pcs"] = Val.ToDecimal(lblOsPcs.Text);
                                    m_dtbLotSplitProcess.Rows[dgvLotSplit.FocusedRowHandle]["os_carat"] = Val.ToDecimal(lblOsCarat.Text);
                                    m_flag = 0;
                                    break;
                                }
                            }
