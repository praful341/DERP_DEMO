...
{
    public partial class FrmMFGFactorySplit : DevExpress.XtraEditors.XtraForm
    {
        #region Data Member
        ...
        private bool ValidateDetails()
        {
            ...
            try
            {
                ...
                if (m_blnadd)
                {
                    ...
                    if (btnAdd.Text == "&Add")
                    {
                        if (Val.ToDecimal(lblTotalCarat.Text) < Val.ToDecimal(clmCarat.SummaryItem.SummaryValue) + Val.ToDecimal(txtCarat.Text))
                        {
                    ...
                    {
                        if (Val.ToDecimal(lblTotalCarat.Text) < (Val.ToDecimal(clmCarat.SummaryItem.SummaryValue) + Val.ToDecimal(txtCarat.Text)) - m_updateCarat)
                        {
                            lstError.Add(new ListError(5, "Carat not greater than balance carat."));
                            if (!blnFocus)
                            {
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (DataRow drw in m_dtbLotMixSplit.Rows)
                    {
                        ...
                        //SrNo = objMFGFactorySplit.GetSrNo(Val.ToInt(drw["rough_cut_id"]), DLL.GlobalDec.EnumTran.Continue, Conn);
                        ...
                        objMFGFactorySplitProperty = objMFGFactorySplit.Save(objMFGFactorySplitProperty, DLL.GlobalDec.EnumTran.Continue, Conn);
