...
namespace BLL.FunctionClasses.Transaction
{
    public class MFGKapanCreate
    {
        ...
        {
            Validation Val = new Validation();
            return Val.ToString(Ope.FindText(BLL.DBConnections.ConnectionString, BLL.DBConnections.ProviderName, "MFG_TRN_Rough_Cut cut left join MFG_TRN_Kapan_Create kpn on kpn.kapan_id = cut.kapan_id", "rough_cut_no", "AND rough_cut_no = '" + CutNo + "' AND kpn.kapan_no = '" + KapanNo + "'"));
        }
